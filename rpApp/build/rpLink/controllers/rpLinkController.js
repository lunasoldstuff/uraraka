...
(function () {
	...
	angular.module('rpLink').controller('rpLinkCtrl', ['$scope', '$rootScope', '$timeout', '$filter', '$mdPanel', 'rpAppLocationService', 'rpSettingsService', rpLinkCtrl]);

	function rpLinkCtrl($scope, $rootScope, $timeout, $filter, $mdPanel, rpAppLocationService, rpSettingsService) {
		...
		if ($scope.post.data.thumbnail !== 'default' && $scope.post.data.thumbnail !== 'self' && $scope.post.data.thumbnail !== 'nsfw') {
		...
		function calcWarning() {
			...
			if (angular.isDefined($scope.post.data.title) && angular.isDefined($scope.post.data.over18)) {
				if (($scope.post.data.title.toLowerCase().indexOf('nsfw') > 0 || $scope.post.data.title.toLowerCase().indexOf('gore') > 0 || $scope.post.data.title.toLowerCase().indexOf('nsfl') > 0 || $scope.post.data.over_18) && $scope.showNSFW) {
