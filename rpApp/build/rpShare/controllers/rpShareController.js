...
(function () {
	...
	angular.module('rpShare').controller('rpShareCtrl', ['$scope', '$window', '$filter', '$mdBottomSheet', '$mdDialog', 'rpAppLocationService', 'rpSettingsService', 'rpAppGoogleUrlService', 'rpAppAuthService', 'rpToastService', 'rpAppIsMobileViewService', 'post', rpShareCtrl]);

	function rpShareCtrl($scope, $window, $filter, $mdBottomSheet, $mdDialog, rpAppLocationService, rpSettingsService, rpAppGoogleUrlService, rpAppAuthService, rpToastService, rpAppIsMobileViewService, post) {
			...
			switch ($index) {
				...
				case 3:
					...
					if (shareTitle.length + shareLink.length < 127) {
						...
						$window.open('https://twitter.com/intent/tweet?text=' + encodeURIComponent(text) + ' via @reddup', 'Share with twitter', "height=500,width=500");
					} else {
						...
						rpAppGoogleUrlService(shareLink, function (err, data) {
							if (err) {
								...
								$window.open('https://twitter.com/intent/tweet?text=' + encodeURIComponent(text), 'Share with twitter', "height=500,width=500");
							}
						});
					}
