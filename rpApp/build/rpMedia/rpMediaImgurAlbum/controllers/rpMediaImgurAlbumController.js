...
(function () {
	...
	angular.module('rpMediaImgurAlbum').controller('rpMediaImgurAlbumCtrl', ['$scope', '$rootScope', '$log', '$filter', '$routeParams', '$mdPanel', 'rpMediaImgurAlbumResourceService', 'rpMediaImgurGalleryResourceService', 'rpMediaImgurAlbumPreloaderService', rpMediaImgurAlbumCtrl]);

	function rpMediaImgurAlbumCtrl($scope, $rootScope, $log, $filter, $routeParams, $mdPanel, rpMediaImgurAlbumResourceService, rpMediaImgurGalleryResourceService, rpMediaImgurAlbumPreloaderService) {
		...
		if (id.indexOf(',') > 0) {
			//implicit album (comma seperated list of image ids)
		...
		$scope.prev = function () {
			...
			if (--imageIndex < 0) imageIndex = n - 1;
			setCurrentImage();
		};
		...
		function preloadImages(images) {
			...
			if (images && images !== 'undefined') {
				...
				rpMediaImgurAlbumPreloaderService.preloadImages(imageLocations).then(function handleResolve(imageLocations) {
