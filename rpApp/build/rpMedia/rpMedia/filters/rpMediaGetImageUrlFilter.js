...
(function () {
	...
	angular.module('rpMedia').filter('rpMediaGetImageUrlFilter', ['$filter', rpMediaGetImageUrlFilter]);
	...
	function rpMediaGetImageUrlFilter($filter) {
		return function (post) {
			...
			if (angular.isUndefined(imageUrl) && angular.isDefined(url)) {
				if (url.substr(url.length - 4) === '.jpg' || url.substr(url.length - 5) === '.jpeg' || url.substr(url.length - 4) === '.png' || url.substr(url.length - 4) === '.bmp') {
					imageUrl = url;
				}
			}
