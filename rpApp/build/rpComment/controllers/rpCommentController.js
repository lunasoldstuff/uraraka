...
(function () {
	...
	angular.module('rpComment').controller('rpCommentCtrl', ['$scope', '$rootScope', '$element', '$compile', '$filter', '$timeout', 'rpCommentChildrenService', 'rpIdentityService', 'rpAppAuthService', 'rpCommentCommentsService', rpCommentCtrl]);

	function rpCommentCtrl($scope, $rootScope, $element, $compile, $filter, $timeout, rpCommentChildrenService, rpIdentityService, rpAppAuthService, rpCommentCommentsService) {
   ...
   */
		...
		$scope.isDeleted = $scope.comment && $scope.comment.data.author !== undefined && $scope.comment.data.body !== undefined && $scope.comment.data.author === '[deleted]' && $scope.comment.data.body === '[deleted]';
		...
		$scope.hasChildren = function () {
			return angular.isDefined($scope.comment.data.replies) && $scope.comment.data.replies !== '' && $scope.comment.data.replies.data.children.length !== 0;
		};
   ...
   * */
		...
		$scope.showMore = function () {
			...
			rpCommentChildrenService($scope.sort, $scope.post.data.name, $scope.comment.data.children.join(","), function (err, data) {
				$scope.isLoadingMoreChildren = false;
				$timeout(angular.noop, 0);
		...
		function reloadComment(callback) {
			...
			rpCommentCommentsService($scope.comment.data.subreddit, $filter('rpAppNameToId36Filter')($scope.comment.data.link_id), 'hot', $scope.comment.data.id, 0, function (err, data) {
				if (err) {
