...
(function () {
	...
	angular.module('rpSlideshow').controller('rpSlideshowCtrl', ['$scope', '$rootScope', '$timeout', '$compile', '$mdPanel', 'rpSettingsService', rpSlideshowCtrl]);

	function rpSlideshowCtrl($scope, $rootScope, $timeout, $compile, $mdPanel, rpSettingsService) {
		...
		$scope.recompile = function () {
			console.log('[rpSlideshow] link, recompile(), typeof newScope ' + (typeof newScope === 'undefined' ? 'undefined' : _typeof(newScope)));
		...
		$scope.openSettings = function ($event) {
			...
			var position = $mdPanel.newPanelPosition().relativeTo('.rp-slideshow-settings-button').addPanelPosition($mdPanel.xPosition.ALIGN_START, $mdPanel.yPosition.BELOW);
